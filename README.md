...
`assets/motion.js` progressively enhances all 27 pages with staged entrances, pointer-aware card depth, magnetic buttons, menu/FAQ feedback, form focus states and reading progress. Motion can be reduced with an on-page control and follows the system's reduced-motion preference. Content remains available without animation support; scrolling stays native. Offline behavior checks: `node scripts/check_motion_behavior.cjs`. These tests do not substitute for browser visual review. No live production check or deployment.
