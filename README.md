...
`assets/motion.js` progressively enhances all 27 pages with staged entrances, CSS-based hover feedback, menu/FAQ feedback, form focus states and reading progress. Pointer tracking no longer creates new transform animations on every mouse move; the header keeps a constant layout height while scrolling. Motion can be reduced with an on-page control and follows the system's reduced-motion preference. Content remains available without animation support; scrolling stays native. Offline behavior checks: `node scripts/check_motion_behavior.cjs`. These tests do not substitute for browser visual review. No live production check or deployment.
